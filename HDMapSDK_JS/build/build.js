...
exports.build = function (callback, version, compsBase32, buildName) {
	...
	var copy = fs.readFileSync('src/copyright.js', 'utf8').replace('{VERSION}', version),
	    ...
	    filenamePart = 'hdmap' + (buildName ? '-' + buildName : ''),
	    pathPart = 'dist/' + filenamePart,
	    srcPath = pathPart + '-src.js',
	    mapPath = pathPart + '-src.map',
		srcPathWS = filenamePart + '-src.js',
		mapPathWS = filenamePart + '-src.map',
	...
	if (newSrc !== oldSrc) {
		...
		fs.writeFileSync(srcPathWS, newSrc);
		fs.writeFileSync(mapPathWS, bundle.generateMap({
			file: srcFilename,
			includeContent: true,
			hires: false
		}));
		console.log('\tSaved to ' + srcPathWS + ' for WebStrom Project');
	}
