describe("Control", function () {
	var map;

	beforeEach(function () {
		map = HDMap.map(document.createElement('div'));
	});

	function onAdd() {
		return HDMap.DomUtil.create('div', 'hdmap-test-control');
	}

	describe("#addTo", function () {
		it("adds the container to the map", function () {
			var control = new HDMap.Control();
			control.onAdd = onAdd;
			control.addTo(map);
			expect(map.getContainer().querySelector('.hdmap-test-control')).to.equal(control.getContainer());
		});

		it("removes the control from any existing map", function () {
			var control = new HDMap.Control();
			control.onAdd = onAdd;
			control.addTo(map);
			control.addTo(map);
			expect(map.getContainer().querySelectorAll('.hdmap-test-control').length).to.equal(1);
			expect(map.getContainer().querySelector('.hdmap-test-control')).to.equal(control.getContainer());
		});
	});

	describe("#remove", function () {
		it("removes the container from the map", function () {
			var control = new HDMap.Control();
			control.onAdd = onAdd;
			control.addTo(map).remove();
			expect(map.getContainer().querySelector('.hdmap-test-control')).to.equal(null);
		});

		it("calls onRemove if defined", function () {
			var control = new HDMap.Control();
			control.onAdd = onAdd;
			control.onRemove = sinon.spy();
			control.addTo(map).remove();
			expect(control.onRemove.called).to.be(true);
		});

		it("is a no-op if the control has not been added", function () {
			var control = new HDMap.Control();
			expect(control.remove()).to.equal(control);
		});
	});
});
