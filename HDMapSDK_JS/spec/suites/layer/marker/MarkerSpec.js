describe("Marker", function () {
	...
	beforeEach(function () {
		...
		map = HDMap.map(div).setView([0, 0], 0);
		icon1 = new HDMap.Icon.Default();
		icon2 = new HDMap.Icon.Default({
	...
	describe("#setIcon", function () {
		it("changes the icon to another image", function () {
			var marker = new HDMap.Marker([0, 0], {icon: icon1});
			map.addLayer(marker);
		...
		it("changes the icon to another DivIcon", function () {
			var marker = new HDMap.Marker([0, 0], {icon: new HDMap.DivIcon({html: 'Inner1Text'})});
			map.addLayer(marker);

			var beforeIcon = marker._icon;
			marker.setIcon(new HDMap.DivIcon({html: 'Inner2Text'}));
			var afterIcon = marker._icon;
		...
		it("removes text when changing to a blank DivIcon", function () {
			...
			marker.setIcon(new HDMap.DivIcon());
			var afterIcon = marker._icon;
		...
		it("changes an image to a DivIcon", function () {
			...
			marker.setIcon(new HDMap.DivIcon({html: 'Inner1Text'}));
	...
	describe("#setLatLng", function () {
		it("fires a move event", function () {
			...
			var afterLatLng = new HDMap.LatLng(1, 2);
	...
	describe('events', function () {
		it('fires click event when clicked', function () {
			...
			var marker = HDMap.marker([0, 0]).addTo(map);
		...
		it('fires click event when clicked with DivIcon', function () {
			...
			var marker = HDMap.marker([0, 0], {icon: new HDMap.DivIcon()}).addTo(map);
		...
		it('fires click event when clicked on DivIcon child element', function () {
			...
			var marker = HDMap.marker([0, 0], {icon: new HDMap.DivIcon({html: '<img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" />'})}).addTo(map);
		...
		it('fires click event when clicked on DivIcon child element set using setIcon', function () {
			...
			var marker = HDMap.marker([0, 0]).addTo(map);
			marker.setIcon(new HDMap.DivIcon({html: '<img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" />'}));
