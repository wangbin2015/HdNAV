describe('Polyline', function () {
	...
	describe("#initialize", function () {
		it("doesn't overwrite the given latlng array", function () {
			...
			var polyline = new HDMap.Polyline(sourceLatLngs);
		...
		it("should accept a multi", function () {
			...
			var polyline = new HDMap.Polyline(latLngs);

			expect(polyline._latlngs[0]).to.eql([HDMap.latLng([1, 2]), HDMap.latLng([3, 4]), HDMap.latLng([5, 6])]);
			expect(polyline._latlngs[1]).to.eql([HDMap.latLng([11, 12]), HDMap.latLng([13, 14]), HDMap.latLng([15, 16])]);
			expect(polyline.getLatLngs()).to.eql(polyline._latlngs);
		});

		it("should accept an empty array", function () {

			var polyline = new HDMap.Polyline([]);
		...
		it("can be added to the map when empty", function () {
			var polyline = new HDMap.Polyline([]).addTo(map);
	...
	describe('#getCenter', function () {
		...
		it('should compute center of a big flat line on equator', function () {
			var polyline = new HDMap.Polyline([[0, 0], [0, 90]]).addTo(map);
			expect(polyline.getCenter()).to.eql(HDMap.latLng([0, 45]));
		});

		it('should compute center of a big flat line close to the pole', function () {
			var polyline = new HDMap.Polyline([[80, 0], [80, 90]]).addTo(map);
			expect(polyline.getCenter()).to.be.nearLatLng(HDMap.latLng([80, 45]), 1e-2);
		});

		it('should compute center of a big diagonal line', function () {
			var polyline = new HDMap.Polyline([[0, 0], [80, 80]]).addTo(map);
			expect(polyline.getCenter()).to.be.nearLatLng(HDMap.latLng([57, 40]), 1);
		});

		it('should compute center of a diagonal line close to the pole', function () {
			var polyline = new HDMap.Polyline([[70, 70], [84, 84]]).addTo(map);
			expect(polyline.getCenter()).to.be.nearLatLng(HDMap.latLng([79, 77]), 1);
		});

		it('should compute center of a big multiline', function () {
			var polyline = new HDMap.Polyline([[10, -80], [0, 0], [0, 10], [10, 90]]).addTo(map);
			expect(polyline.getCenter()).to.be.nearLatLng(HDMap.latLng([0, 5]), 1);
		});

		it('should compute center of a small flat line', function () {
			var polyline = new HDMap.Polyline([[0, 0], [0, 0.090]]).addTo(map);
			map.setZoom(0);  // Make the line disappear in screen;
			expect(polyline.getCenter()).to.be.nearLatLng(HDMap.latLng([0, 0]), 1e-2);
		});

	});

	describe('#_flat', function () {
		var layer = HDMap.polyline([]);

		it('should return true for an array of LatLngs', function () {
			expect(HDMap.Polyline._flat([HDMap.latLng([0, 0])])).to.be(true);
		});

		it('should return true for an array of LatLngs arrays', function () {
			expect(HDMap.Polyline._flat([[0, 0]])).to.be(true);
		});

		it('should return true for an empty array', function () {
			expect(HDMap.Polyline._flat([])).to.be(true);
		});

		it('should return false for a nested array of LatLngs', function () {
			expect(HDMap.Polyline._flat([[HDMap.latLng([0, 0])]])).to.be(false);
		});

		it('should return false for a nested empty array', function () {
			expect(HDMap.Polyline._flat([[]])).to.be(false);
		});
	...
	describe("#_defaultShape", function () {
		...
		it("should return latlngs when flat", function () {
			var latLngs = [HDMap.latLng([1, 2]), HDMap.latLng([3, 4])];
		...
		it("should return first latlngs on a multi", function () {
			var latLngs = [
				[HDMap.latLng([1, 2]), HDMap.latLng([3, 4])],
				[HDMap.latLng([11, 12]), HDMap.latLng([13, 14])]
			];
	...
	describe("#addLatLng", function () {
		...
		it("should add latlng to latlngs", function () {
			...
			expect(polyline._latlngs).to.eql([HDMap.latLng([1, 2]), HDMap.latLng([3, 4]), HDMap.latLng([5, 6])]);
		});
		...
		it("should add latlng to first latlngs on a multi", function () {
			...
			expect(polyline._latlngs[0]).to.eql([HDMap.latLng([1, 2]), HDMap.latLng([3, 4]), HDMap.latLng([5, 6])]);
			expect(polyline._latlngs[1]).to.eql([HDMap.latLng([11, 12]), HDMap.latLng([13, 14])]);
		});
		...
		it("should add latlng to latlngs by reference", function () {
			...
			expect(polyline._latlngs[1]).to.eql([HDMap.latLng([1, 2]), HDMap.latLng([3, 4]), HDMap.latLng([5, 6])]);
			expect(polyline._latlngs[0]).to.eql([HDMap.latLng([11, 12]), HDMap.latLng([13, 14])]);
		});
		...
		it("should add latlng on empty polyline", function () {
			...
			expect(polyline._latlngs).to.eql([HDMap.latLng([1, 2])]);
		});
