describe('LatLngBounds', function () {
	...
	beforeEach(function () {
		a = new HDMap.LatLngBounds(
			new HDMap.LatLng(14, 12),
			new HDMap.LatLng(30, 40));
		c = new HDMap.LatLngBounds();
	});

	describe('constructor', function () {
		it('instantiates either passing two latlngs or an array of latlngs', function () {
			var b = new HDMap.LatLngBounds([
				new HDMap.LatLng(14, 12),
				new HDMap.LatLng(30, 40)
			]);
			expect(b).to.eql(a);
			expect(b.getNorthWest()).to.eql(new HDMap.LatLng(30, 12));
		});
	});

	describe('#extend', function () {
		it('extends the bounds by a given point', function () {
			a.extend(new HDMap.LatLng(20, 50));
			expect(a.getNorthEast()).to.eql(new HDMap.LatLng(30, 50));
		});

		it('extends the bounds by given bounds', function () {
			a.extend([[20, 50], [8, 40]]);
			expect(a.getSouthEast()).to.eql(new HDMap.LatLng(8, 50));
		});
		...
		it('extend the bounds by an empty bounds object', function () {
			expect(a.extend(new HDMap.LatLngBounds())).to.eql(a);
		});
	});

	describe('#getCenter', function () {
		it('returns the bounds center', function () {
			expect(a.getCenter()).to.eql(new HDMap.LatLng(22, 26));
		});
	});
	...
	describe('#pad', function () {
		it('pads the bounds by a given ratio', function () {
			...
			expect(b).to.eql(HDMap.latLngBounds([[6, -2], [38, 54]]));
		});
	});
	...
	describe('#getNorthWest', function () {
		it('returns a proper north-west LatLng', function () {
			expect(a.getNorthWest()).to.eql(new HDMap.LatLng(a.getNorth(), a.getWest()));
		});
	...
	describe('#getSouthEast', function () {
		it('returns a proper south-east LatLng', function () {
			expect(a.getSouthEast()).to.eql(new HDMap.LatLng(a.getSouth(), a.getEast()));
		});
	});
	...
	describe('#contains', function () {
		it('returns true if contains latlng point', function () {
			...
			expect(HDMap.latLngBounds(a).contains([5, 20])).to.eql(false);
		});
