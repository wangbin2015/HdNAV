describe('LatLng', function () {
	describe('constructor', function () {
		it("sets lat and lng", function () {
			var a = new HDMap.LatLng(25, 74);
			expect(a.lat).to.eql(25);
			expect(a.lng).to.eql(74);

			var b = new HDMap.LatLng(-25, -74);
		...
		it('throws an error if invalid lat or lng', function () {
			expect(function () {
				var a = new HDMap.LatLng(NaN, NaN);
			}).to.throwError();
		});
		...
		it('sets altitude', function () {
			var a = new HDMap.LatLng(25, 74, 50);
			expect(a.alt).to.eql(50);

			var b = new HDMap.LatLng(-25, -74, -50);
	...
	describe('#toString', function () {
		it('formats a string', function () {
			var a = new HDMap.LatLng(10.333333333, 20.2222222);
	...
	describe('#distanceTo', function () {
		it('calculates distance in meters', function () {
			var a = new HDMap.LatLng(50.5, 30.5);
			var b = new HDMap.LatLng(50, 1);
		...
		it('does not return NaN if input points are equal', function () {
			var a = new HDMap.LatLng(50.5, 30.5);
			var b = new HDMap.LatLng(50.5, 30.5);

			expect(a.distanceTo(b)).to.eql(0);
		});
	});

	describe('HDMap.latLng factory', function () {
		it('returns LatLng instance as is', function () {
			var a = new HDMap.LatLng(50, 30);

			expect(HDMap.latLng(a)).to.eql(a);
		});

		it('accepts an array of coordinates', function () {
			expect(HDMap.latLng([])).to.eql(null);
			expect(HDMap.latLng([50])).to.eql(null);
			expect(HDMap.latLng([50, 30])).to.eql(new HDMap.LatLng(50, 30));
			expect(HDMap.latLng([50, 30, 100])).to.eql(new HDMap.LatLng(50, 30, 100));
		});

		it('passes null or undefined as is', function () {
			expect(HDMap.latLng(undefined)).to.eql(undefined);
			expect(HDMap.latLng(null)).to.eql(null);
		});
		...
		it('accepts an object with lat/lng', function () {
			expect(HDMap.latLng({lat: 50, lng: 30})).to.eql(new HDMap.LatLng(50, 30));
		});

		it('accepts an object with lat/lon', function () {
			expect(HDMap.latLng({lat: 50, lon: 30})).to.eql(new HDMap.LatLng(50, 30));
		});

		it('returns null if lng not specified', function () {
			expect(HDMap.latLng(50)).to.be(null);
		});
		...
		it('accepts an object with alt', function () {
			expect(HDMap.latLng({lat: 50, lng: 30, alt: 100})).to.eql(new HDMap.LatLng(50, 30, 100));
			expect(HDMap.latLng({lat: 50, lon: 30, alt: 100})).to.eql(new HDMap.LatLng(50, 30, 100));
		});
	});
	...
	describe('#clone', function () {
		...
		it('should clone attributes', function () {
			var a = new HDMap.LatLng(50.5, 30.5, 100);
			var b = a.clone();
