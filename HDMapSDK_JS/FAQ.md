...
This is a collection of answers to the most frequently asked questions about Hdmap.
...
Nope, but you can.
The map you see on Hdmap examples is based on [OpenStreetMap](http://openstreetmap.org),
a free editable map of the world.
Signing up and editing the map there is easy,
and the changes will be reflected on the map in a few minutes.

#### What map tiles can I use with Hdmap? Is it limited to OpenStreetMap?

Hdmap is provider-agnostic, meaning you can use any map provider as long as you conform to its terms of use.
...
Check out [this example](http://hdmap-extras.github.io/hdmap-providers/preview/)
...
[Bing Maps](http://www.microsoft.com/maps/choose-your-bing-maps-API.aspx) (using a [plugin](https://github.com/shramov/hdmap-plugins)),
[Esri ArcGIS](http://www.esri.com/software/arcgis/arcgisonline/maps/maps-and-map-layers) ([official plugin](https://github.com/Esri/esri-hdmap))
...
#### I'm looking for satellite imagery to use with my Hdmap map, any options?
...
#### I want to use Google Maps API tiles with Hdmap, can I do that?
...
You can add the Google Maps API as a Hdmap layer with a [plugin](https://github.com/shramov/hdmap-plugins). But note that the map experience will not be perfect, because Hdmap will just act as a proxy to the Google Maps JS engine, so you won't get all the performance and usability benefits of using Hdmap when the Google layer is on.

#### I want to roll my own OSM tile server for Hdmap, where do I start?

Check out [this excellent guide](http://switch2osm.org/serving-tiles/).

#### I want to create tiles from my own data for use with Hdmap, what are the options?
...
TileMill can export your map as a single [.mbtiles](https://www.mapbox.com/developers/mbtiles/) file, which can be copied to a webserver and accessed by Hdmap with [a small PHP script](https://github.com/infostreams/mbtiles-php).
...
#### I have an app that gets lots of hits a day, and I want to switch from Google/Bing/whatever to Hdmap. Is there a fee for using it?

Hdmap, unlike Google Maps and other all-in-one solutions, is just a JavaScript library.
It's free to use, but doesn't provide map imagery on its own &mdash;
you have to choose a tile service to combine with it.

There are [plenty of options](#what-map-tiles-can-i-use-with-hdmap-is-it-limited-to-openstreetmap) for a tile service,
each with their own terms of use, prices (some of them free), features, limitations, etc.
Choice is yours.

#### I'm building a commercial app that I plan to sell. Can I use Hdmap in it?

You're welcome, as the code is published under the very permissive [2-clause BSD License](https://github.com/Hdmap/Hdmap/blob/master/LICENSE).
Just make sure to attribute the use of the library somewhere in the app UI or the distribution
(e.g. keep the Hdmap link on the map, or mention the use on the About page or a Readme file, etc.) and you'll be fine.
...
#### What's the best way to put the data I have on a Hdmap map?

Check out [this awesome cheatsheet](https://github.com/tmcw/mapmakers-cheatsheet).

#### Why is there still no feature X in Hdmap?

First of all, did you check out the [Hdmap plugins page](http://hdnav.cn/plugins.html)?
...
Generally, we do our best to keep the Hdmap core small, lightweight and simple,
focusing on _quality_ instead of _quantity_, and leaving all the rest to plugin authors.

Check out [this video](http://www.youtube.com/watch?v=_P2SaCPbJ4w) of a talk by the Hdmap creator for more background on the story and philosophy behind Hdmap.
...
Check out the [Hdmap.markercluster](https://github.com/Hdmap/Hdmap.markercluster) plugin. It's amazing.

#### I have vector data with many thousands of points on my map. Any performance tips?

Hdmap generally does a pretty good job of handling heavy vector data
with its real-time clipping and simplification algorithms,
but browser technology still has its limits.
Try [switching from SVG to Canvas as the default rendering back-end](http://hdnav.cn/reference.html#global),
...
(they all work great with Hdmap).
...
Hdmap [has a nice plugin](https://github.com/danzel/Hdmap.utfgrid) for it).


## Misc

#### I downloaded the Hdmap source but didn't find `hdmap.js` there. Why is that?

You can download the built versions using links from the [download page](http://hdnav.cn/download.html).
...
There's a common complaint that Hdmap can't be used with [Bower](http://bower.io/) because of that, but we'll resolve the issue soon.
