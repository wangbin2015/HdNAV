# Hdmap Plugin Authoring Guide

One of the greatest things about Hdmap is its powerful plugin ecosystem.
The [Hdmap plugins page](http://hdnav.cn/plugins.html) lists dozens of awesome plugins, and more are being added every week.

This guide lists a number of best practices for publishing a Hdmap plugin that meets the quality standards of Hdmap itself.
...
The best place to put your Hdmap plugin to is a separate [GitHub](http://github.com) repository.
...
Most existing plugins follow the convention of naming plugins (and repos) like this: `Hdmap.MyPluginName`.
You can use other forms (e.g. "hdmap-my-plugin-name"),
just make sure to include the word "Hdmap" in the name so that it's obvious that it's a Hdmap plugin.
...
Hdmap follows pretty much the same conventions
...
If you have a new class, put it directly in the `HDMap` namespace (`HDMap.MyPlugin`).<br>
If you inherit one of the existing classes, make it a sub-property (`HDMap.TileLayer.Banana`).<br>
If you want to add new methods to existing Hdmap classes, you can do it like this: `HDMap.Marker.include({myPlugin: …})`.
...
And most importantly, keep it simple. Hdmap is all about *simplicity*.
...
When you publish your plugin you should add a depenency on `hdmap` to your `package.json` file. This will automatically install Hdmap when your package is installed.

Here is an example of a `package.json` file for a Hdmap plugin.

```json
{
  "name": "my-hdmap-plugin",
  "version": "1.0.0",
  "description": "A simple hdmap plugin.",
  "main": "my-plugin.js",
  "author": "You",
  "license": "IST",
  "dependencies": {
...
You can add support for AMD/CommonJS loaders to your Hdmap plugin by following this pattern based on the [Universal Module  Definition](https://github.com/umdjs/umd/blob/master/returnExportsGlobal.js)

```js
(function (factory, window) {

    // define an AMD module that relies on 'hdmap'
    if (typeof define === 'function' && define.amd) {
        define(['hdmap'], factory);
    
    // define a Common JS module that relies on 'hdmap'
    } else if (typeof exports === 'object') {
        module.exports = factory(require('hdmap'));
    }

    // attach your plugin to the global 'HDMap' variable
    if (typeof window !== 'undefined' && window.HDMap) {
        window.HDMap.YourPlugin = factory(HDMap);
    }
}(function (HDMap) {
    var MyLeafletPlugin = {};
