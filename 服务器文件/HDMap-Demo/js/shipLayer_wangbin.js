HDMap.Marker = HDMap.Layer.extend({
	...
	addTo : function(map){
		...
		if(_lon != null || _lat != null || _height != null || _width != null){
			shipSelected(_lon,_lat,_height,_width);
		};
		if(_shipLayerLoaded){
			_shipLayerLoaded = false;
			_map.on('click',function(e){
				if(_shipSelected != null){
...
function shipSelected(lon,lat,height,width) {
	_lon = lon;
	_lat = lat;
	_height = height;
	_width = width;
...
function drawnShip(zoom,lnglat,width,height,angle,speed,shipType,shipName,mmsi) {
	//angle = 0;
	var cbJson;
	//对船速线的处理
	var speedLineLength;
	...
	if (((zoom == 14 && height >= 0.2) || (zoom == 15 && height >= 0.1) || (zoom == 16 && height >= 0.05) || (zoom == 17 && height >= 0.03) || (zoom == 18 && height >= 0.01)) && height/width > 2 && (height > 0 && height < 0.5) && (width > 0 && width < 0.2)) {
		...
		latLng1 = calculateCoordinate(lnglat, speedLineLength + 0.5 * height , angle);
		latLng2 = calculateCoordinate(lnglat, 0.5 * height , angle);
		latLng3 = calculateCoordinate(lnglat, Math.sqrt(4/9*height*4/9*height + 0.25*width*0.25*width) , Math.atan(0.25*width/(4/9*height))*180/Math.PI + angle);
		latLng4 = calculateCoordinate(lnglat, Math.sqrt(1/3*height*1/3*height + 0.5*width*0.5*width) , Math.atan(0.5*width/(1/3*height))*180/Math.PI + angle);
		latLng5 = calculateCoordinate(lnglat, Math.sqrt(5/12*height*5/12*height + 0.5*width*0.5*width) , 180 - (Math.atan(0.5*width/(5/12*height))*180/Math.PI) + angle);
		latLng6 = calculateCoordinate(lnglat, Math.sqrt(0.5*height*0.5*height + 1/3*width*1/3*width) , 180 - (Math.atan(1/3*width/(0.5*height))*180/Math.PI) + angle);
		latLng7 = calculateCoordinate(lnglat, Math.sqrt(0.5*height*0.5*height + 1/3*width*1/3*width) , 180 + (Math.atan(1/3*width/(0.5*height))*180/Math.PI) + angle);
		latLng8 = calculateCoordinate(lnglat, Math.sqrt(5/12*height*5/12*height + 0.5*width*0.5*width) , 180 + (Math.atan(0.5*width/(5/12*height))*180/Math.PI) + angle);
		latLng9 = calculateCoordinate(lnglat, Math.sqrt(1/3*height*1/3*height + 0.5*width*0.5*width) , 360 - (Math.atan(0.5*width/(1/3*height))*180/Math.PI) + angle);
		latLng10 = calculateCoordinate(lnglat, Math.sqrt(4/9*height*4/9*height + 0.25*width*0.25*width) , 360 - (Math.atan(0.25*width/(4/9*height))*180/Math.PI) + angle);
		cbJson = {
			"type": "Feature",
			"geometry": {
				"type": "Polygon",
		...
		//获得中心点的屏幕坐标
		var pointCenter = _map.latLngToContainerPoint(HDMap.latLng(lnglat.lat, lnglat.lng));
		var point1 = HDMap.point(pointCenter.x, pointCenter.y-11-speedLineLength);
		var point2 = HDMap.point(pointCenter.x, pointCenter.y-11);
		var point3 = HDMap.point(pointCenter.x+5, pointCenter.y+11);
		var point4 = HDMap.point(pointCenter.x-5, pointCenter.y+11);
		...
		latLng1 = calculateCoordinate(lnglat, lnglat.distanceTo(latLng1)/1000 , angle);
		latLng2 = calculateCoordinate(lnglat, lnglat.distanceTo(latLng2)/1000 , angle);
		latLng3 = calculateCoordinate(lnglat, lnglat.distanceTo(latLng3)/1000 , 151.4281218577 + angle);
		latLng4 = calculateCoordinate(lnglat, lnglat.distanceTo(latLng4)/1000 , 208.5718781423 + angle);
		cbJson = {
			"type": "Feature",
			"geometry": {
				"type": "Polygon",
...
HDMap.ship = function (map, data) {
	return new HDMap.Marker(map, data);
};
function calculateCoordinate(lnglat, distance, angle){
	//计算点的经纬度坐标
	var calc_point;
